...
def main_train(train_dataset, validation_dataset, extra_dataset=None):
    ...
    # SINCE WE ARE TRAINING OUR OWN FUNCTION FROM SCRATCH LETS ASSUME WE ARE NOT USING A PRETRAINED MODEL
    elif args.load:
    ...
    for epoch in range(args.start_epoch + 1, args.epochs + 1):
        ...
        model.train() # sets the model mode to train
...
def train_epoch(epoch, trainer, train_dataloader, meters):
    ...
    with tqdm_pbar(total=nr_iters) as pbar:
        for i in range(nr_iters):
            feed_dict = next(train_iter) # parses info from scene image question into a dict see getitem
